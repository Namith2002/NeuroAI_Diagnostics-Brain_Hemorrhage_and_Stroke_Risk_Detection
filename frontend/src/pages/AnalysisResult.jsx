...
<<<<<<< HEAD
  const getClassificationData = () => {
    if (!report) return [];
    const subtypes = [
      { key: "prob_sah", label: "Subarachnoid (SAH)", color: "bg-red-400" },
      { key: "prob_edh", label: "Epidural (EDH)", color: "bg-cyan-400" },
      { key: "prob_ivh", label: "Intraventricular (IVH)", color: "bg-emerald-400" },
      { key: "prob_iph", label: "Intraparenchymal (IPH)", color: "bg-purple-400" },
      { key: "prob_sdh", label: "Subdural (SDH)", color: "bg-amber-400" },
      { key: "prob_fracture", label: "Fracture (FRAC)", color: "bg-fuchsia-400" }
    ];

    const overallHemorrhage = {
      label: "Hemorrhage (Overall)",
      prob: (report.prob_hemorrhage || 0.0) / 100,
      confidence: report.prob_hemorrhage || 0.0,
      color: "bg-rose-500",
      isOverall: true
    };
    ...
    return [overallHemorrhage, ...sortedSubtypes];
  };

  const getStrokeData = () => {
    if (!report) return [];
    const strokeConfidence = report.stroke_risk || 0.0;
    const normalConfidence = 100.0 - strokeConfidence;

    const rows = [
      {
        label: "Normal",
        prob: normalConfidence / 100,
        confidence: normalConfidence,
        color: "bg-emerald-500"
      },
      {
        label: "Stroke",
        prob: strokeConfidence / 100,
                    ...
                    <table className="w-full border-collapse text-left">
                      ...
                      <tbody>
                        {getClassificationData().map((row) => {
                          ...
                          return (
                            <tr key={row.label} className="border-b border-slate-900/60 hover:bg-slate-900/20 transition-colors duration-150">
                              <td className="py-2.5 px-3 text-[10px] font-bold text-slate-200 flex items-center gap-2">
...
<<<<<<< HEAD
                  <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse">
                      <thead>
                        <tr className="bg-slate-900/60 border-b border-slate-800">
                          <th className="py-2 px-3 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Class</th>
                          <th className="text-right py-2 px-3 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Probability</th>
                          <th className="text-right py-2 px-3 text-[9px] font-bold text-slate-400 uppercase tracking-wider">Confidence</th>
                        </tr>
                      </thead>
                      <tbody>
                        {getStrokeData().map((row) => (
                          <tr key={row.label} className="border-b border-slate-900/60 hover:bg-slate-900/20 transition-colors duration-150">
                            <td className="py-2.5 px-3 text-[10px] font-bold text-slate-200 flex items-center gap-2">
                              <span className={`w-1.5 h-1.5 rounded-full ${row.color}`}></span>
                              <span>{row.label}</span>
              ...
              <div className="p-6 rounded-3xl bg-panelBg/40 border border-panelBorder flex flex-col gap-6">
                <div>
                  <h3 className="text-sm font-black text-white uppercase tracking-wider mb-1 flex items-center gap-2">
                    ...
                    Clinical Assessment Engine (CAE)
                  </h3>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                    Calculated clinical metrics and risk indices derived from multi-label scan features
                  </p>
                </div>

                <div className="grid lg:grid-cols-12 gap-6 items-stretch">
                  {/* Left Column: IDI, HER, SRS Gauges & Main Triage */}
                  <div className="lg:col-span-5 flex flex-col gap-5 justify-between">
                    {/* Intervention Delay Index Card */}
                    <div className="p-5 rounded-2xl bg-slate-950/20 border border-panelBorder/60 flex flex-col gap-4">
                      <div className="flex justify-between items-start">
                        <div className="text-left">
                          <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">Intervention Delay Index (IDI)</span>
                          <div className="text-2xl font-black text-white mt-1">{(report.idi || 0.0).toFixed(4)}</div>
                        </div>
                        <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-black uppercase tracking-wider border ${
                          report.idi >= 7.0 ? 'bg-red-500/10 border-red-500/30 text-rose-400 animate-pulse' :
                          report.idi >= 3.0 ? 'bg-orange-500/10 border-orange-500/30 text-orange-400' :
                          report.idi >= 1.0 ? 'bg-yellow-500/10 border-yellow-500/30 text-yellow-400' :
                          'bg-emerald-500/10 border-emerald-500/30 text-emerald-400'
                        }`}>
                          {report.treatment_recommendation || "Routine (>24 hours)"}
                        </span>
                      </div>
                      ...
                      <div className="w-full bg-slate-900 rounded-full h-2 overflow-hidden border border-slate-800">
                        <div 
                          className={`h-full rounded-full transition-all duration-500 ${
                            report.idi >= 7.0 ? 'bg-gradient-to-r from-red-500 to-rose-600' :
                            report.idi >= 3.0 ? 'bg-gradient-to-r from-orange-400 to-amber-500' :
                            report.idi >= 1.0 ? 'bg-gradient-to-r from-yellow-300 to-yellow-500' :
                            'bg-gradient-to-r from-emerald-400 to-teal-500'
                          }`}
                          style={{ width: `${Math.min(100, (report.idi / 12) * 100)}%` }}
                        ></div>
                      </div>
                      <p className="text-[10px] text-slate-400 font-semibold leading-relaxed text-left">
                        IDI quantifies treatment urgency by weighting hemorrhage type probabilities and presence. Urgent or Immediate indices warrant rapid surgical consultation.
                      </p>
                    </div>

                    {/* HER & SRS Cards */}
                    <div className="grid grid-cols-2 gap-4">
                      <div className="p-4 rounded-xl bg-slate-950/20 border border-panelBorder/60 flex flex-col justify-between gap-2 text-left">
                        <div>
                          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Hematoma Expansion Rate</span>
                          <div className="text-xl font-black text-rose-400 mt-1">{(report.her || 0.0).toFixed(2)}%</div>
                        </div>
                        <p className="text-[9px] text-slate-500 leading-normal font-semibold">
                          Expected bleed volume expansion rate based on acute sub-types.
                        </p>
                      </div>

                      <div className="p-4 rounded-xl bg-slate-950/20 border border-panelBorder/60 flex flex-col justify-between gap-2 text-left">
                        <div>
                          <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider">Seizure Risk Score (SRS)</span>
                          <div className="text-xl font-black text-amber-400 mt-1">{(report.srs || 0.0).toFixed(4)}</div>
                        </div>
                        <p className="text-[9px] text-slate-500 leading-normal font-semibold">
                          Propensity for epilepsy derived from cortical proximity & bleed type.
                        </p>
                      </div>
                    </div>
                  </div>

                  {/* Right Column: Other Clinical Metrics Radar/Bar or Grid Details */}
                  <div className="lg:col-span-7 flex flex-col gap-4 text-left">
                    <div className="p-5 rounded-2xl bg-slate-950/20 border border-panelBorder/60 h-full flex flex-col justify-between gap-4">
                      <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider block">Clinical Indices Breakdown</span>
                      
                      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        {/* ESI Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Edema Severity (ESI)</span>
                          <div className="text-sm font-extrabold text-indigo-400 mt-1">{(report.esi || 0.0).toFixed(4)}</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Scale: 0.0 - 10.0</span>
                        </div>

                        {/* RCF Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Rehab Compliance (RCF)</span>
                          <div className="text-sm font-extrabold text-emerald-400 mt-1">{(report.rcf || 0.0).toFixed(2)}%</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Scale: 0.0 - 100%</span>
                        </div>

                        {/* HI Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Hemosiderin Index (HI)</span>
                          <div className="text-sm font-extrabold text-pink-400 mt-1">{(report.hi || 0.0).toFixed(4)}</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Late degradation product</span>
                        </div>

                        {/* SFS Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Sleep Fragment. (SFS)</span>
                          <div className="text-sm font-extrabold text-cyan-400 mt-1">{(report.sfs || 0.0).toFixed(2)}%</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Sleep disturbance index</span>
                        </div>

                        {/* EV Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Electrolyte Variance (EV)</span>
                          <div className="text-sm font-extrabold text-rose-400 mt-1">{(report.ev || 0.0).toFixed(4)}</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Hyponatremia risk</span>
                        </div>

                        {/* CP Card */}
                        <div className="p-3 bg-slate-900/40 rounded-xl border border-panelBorder/40 text-left">
                          <span className="text-[9px] text-slate-500 font-extrabold uppercase">Cortical Proximity (CP)</span>
                          <div className="text-sm font-extrabold text-amber-400 mt-1">{(report.cp || 0.0).toFixed(4)}</div>
                          <span className="text-[8px] text-slate-600 block mt-0.5">Bleed cortical proximity</span>
                        </div>
                      </div>

                      {/* Dynamic clinical advisory alert based on CP, IDI and ESI */}
                      <div className="p-3.5 bg-slate-900/60 rounded-xl border border-panelBorder/60 text-[10px] text-slate-400 font-semibold leading-relaxed flex items-start gap-2.5">
                        <span className="text-cyan-400 text-sm">💡</span>
                        <div>
                          <span className="text-white font-extrabold block mb-0.5">Integrated Prognostic Insight</span>
                          {report.idi >= 3.0 ? (
                            <span>High Intervention Delay Index with elevated Edema Severity Index (ESI) warrants continuous neurological checks, ICP monitoring, and immediate hyperosmolar therapy consideration.</span>
                          ) : (
                            <span>Clinical metrics indicate a stable cerebral tissue environment. Standard monitoring protocols and routine electrolyte monitoring are recommended.</span>
                          )}
                        </div>
                      </div>
                    </div>
