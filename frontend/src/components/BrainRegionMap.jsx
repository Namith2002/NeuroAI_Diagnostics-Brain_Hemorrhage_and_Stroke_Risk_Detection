...
>>>>>>> fe5fbb4f498c043d0b6c3ff5655b701f2254a367
    ...
    const gradientMap = {
      'Epidural Hematoma': 'url(#activeGradEDH)',
      'Subdural Hematoma': 'url(#activeGradSDH)',
      'Subarachnoid Hemorrhage': 'url(#activeGradSAH)',
      'Intracerebral Hemorrhage': 'url(#activeGradICH)'
    };
    const activeColor = gradientMap[regionKey] || 'url(#activeGradEDH)';
    ...
    <div className="p-5 rounded-2xl bg-panelBg/20 border border-panelBorder flex flex-col gap-6">
      ...
      <div className="w-full flex flex-col items-center justify-center min-h-[220px] relative bg-slate-950/40 rounded-xl border border-panelBorder/60 p-4 overflow-hidden">
        ...
        <svg className="w-full max-w-[480px] h-auto aspect-[480/220] my-2" viewBox="0 0 480 220">
          <defs>
            {/* Active glowing gradients per region */}
            <linearGradient id="activeGradEDH" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#f43f5e" />
              <stop offset="100%" stopColor="#be123c" />
            </linearGradient>
            <linearGradient id="activeGradSDH" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#f97316" />
              <stop offset="100%" stopColor="#b45309" />
            </linearGradient>
            <linearGradient id="activeGradSAH" x1="0%" y1="0%" x2="100%" y2="100%">
              <stop offset="0%" stopColor="#eab308" />
              <stop offset="100%" stopColor="#a16207" />
            </linearGradient>
            <linearGradient id="activeGradICH" x1="0%" y1="0%" x2="100%" y2="100%">
...
<<<<<<< HEAD
          ...
          <g id="grid" className="opacity-15">
            ...
            <circle cx="240" cy="110" r="80" fill="none" stroke="#475569" strokeWidth="0.5" strokeDasharray="2,4" />
            <circle cx="240" cy="110" r="120" fill="none" stroke="#475569" strokeWidth="0.5" strokeDasharray="2,4" />
            <line x1="80" y1="110" x2="400" y2="110" stroke="#475569" strokeWidth="0.5" strokeDasharray="2,4" />
            <line x1="240" y1="10" x2="240" y2="210" stroke="#475569" strokeWidth="0.5" strokeDasharray="2,4" />
          </g>

          {/* Base Anatomical Structure (Always visible in background) */}
          <g id="base-anatomy">
            {/* Skull Bone - Outer Ring */}
            <ellipse cx="240" cy="110" rx="160" ry="100" fill="none" stroke="#475569" strokeWidth="2.5" className="opacity-95" />
            
            {/* Skull Bone - Inner Ring */}
            <ellipse cx="240" cy="110" rx="155" ry="95" fill="#0b0f19" stroke="#334155" strokeWidth="1" className="opacity-95" />
...
<<<<<<< HEAD
            ...
            <path
              ...
              {...getBleedProps('Epidural Hematoma')}
            />
            
            {/* Subdural Hematoma (Crescent shape on the right edge now) */}
            <path
              d="M 368,65 C 400,95 400,135 368,165 C 380,145 380,85 368,65 Z"
              {...getBleedProps('Subdural Hematoma')}
            />
            
            {/* Subarachnoid Hemorrhage (CSF-space sulcal and cistern bleed lines, concentrated bottom-left) */}
            {(() => {
              const isSAHActive = activeKey === 'Subarachnoid Hemorrhage' || (isMultiple && activeKey !== 'Unknown');
              return (
                <path
                  d="M 240,130 L 240,176 M 158,162 C 175,168 190,174 210,170 M 140,152 C 160,158 180,164 200,158 M 150,135 C 165,145 185,150 205,145"
                  className={`transition-all duration-700 ease-in-out ${
                    isSAHActive ? 'animate-pulse opacity-100' : 'opacity-15'
                  }`}
                  fill="none"
                  stroke={isSAHActive ? '#eab308' : '#334155'}
                  strokeWidth={isSAHActive ? 2.5 : 1}
                  strokeDasharray={isSAHActive ? 'none' : '3,3'}
                  filter={isSAHActive ? 'url(#glow)' : 'none'}
                />
              );
            })()}
            
            {/* Intracerebral Hemorrhage (Focal lobulated intraparenchymal bleed on the right side) */}
            <path
...
<<<<<<< HEAD
          <g id="labels" className="text-[8px] font-extrabold uppercase tracking-wider">
            {/* Epidural Space (1) */}
            {(() => {
              const isActive = activeKey === 'Epidural Hematoma' || (isMultiple && activeKey !== 'Unknown');
              return (
                <g className="transition-all duration-500">
                  <text x="20" y="45" textAnchor="start" className={isActive ? 'fill-rose-400' : 'fill-slate-500'}>
                    1. Epidural Space (Biconvex)
                  </text>
                  <path 
                    d="M 140,45 L 125,45 L 110,115" 
                    fill="none"
                    stroke={isActive ? '#be123c' : '#334155'} 
                    strokeWidth={isActive ? 1.5 : 0.8} 
                    strokeDasharray={isActive ? 'none' : '2,2'} 
                    className="opacity-70"
                  />
                  <circle cx="110" cy="115" r={isActive ? 3.5 : 2} fill={isActive ? '#f43f5e' : '#475569'} className="transition-all duration-300" />
                </g>
              );
            })()}
            ...
            {/* Subdural Space (2) */}
            {(() => {
              const isActive = activeKey === 'Subdural Hematoma' || (isMultiple && activeKey !== 'Unknown');
              return (
                <g className="transition-all duration-500">
                  <text x="460" y="45" textAnchor="end" className={isActive ? 'fill-orange-400' : 'fill-slate-500'}>
                    2. Subdural Space (Crescent)
                  </text>
                  <path 
                    d="M 340,45 L 355,45 L 378,115" 
                    fill="none"
                    stroke={isActive ? '#b45309' : '#334155'} 
                    strokeWidth={isActive ? 1.5 : 0.8} 
                    strokeDasharray={isActive ? 'none' : '2,2'} 
                    className="opacity-70"
                  />
                  <circle cx="378" cy="115" r={isActive ? 3.5 : 2} fill={isActive ? '#f97316' : '#475569'} className="transition-all duration-300" />
                </g>
              );
            })()}
            ...
            {(() => {
              const isActive = activeKey === 'Subarachnoid Hemorrhage' || (isMultiple && activeKey !== 'Unknown');
              return (
                <g className="transition-all duration-500">
                  <text x="20" y="185" textAnchor="start" className={isActive ? 'fill-yellow-400' : 'fill-slate-500'}>
                    3. Subarachnoid Space (CSF)
                  </text>
                  <path 
                    d="M 150,185 L 165,185 L 180,165" 
                    fill="none"
                    stroke={isActive ? '#a16207' : '#334155'} 
                    strokeWidth={isActive ? 1.5 : 0.8} 
                    strokeDasharray={isActive ? 'none' : '2,2'} 
                    className="opacity-70"
                  />
                  <circle cx="180" cy="165" r={isActive ? 3.5 : 2} fill={isActive ? '#eab308' : '#475569'} className="transition-all duration-300" />
                </g>
              );
            })()}

            {/* Intracerebral Space (4) */}
            {(() => {
              const isActive = activeKey === 'Intracerebral Hemorrhage' || (isMultiple && activeKey !== 'Unknown');
              return (
                <g className="transition-all duration-500">
                  <text x="460" y="185" textAnchor="end" className={isActive ? 'fill-purple-400' : 'fill-slate-500'}>
                    4. Intracerebral Tissue
                  </text>
                  <path 
                    d="M 340,185 L 325,185 L 285,125" 
                    fill="none"
                    stroke={isActive ? '#6b21a8' : '#334155'} 
                    strokeWidth={isActive ? 1.5 : 0.8} 
                    strokeDasharray={isActive ? 'none' : '2,2'} 
                    className="opacity-70"
                  />
                  <circle cx="285" cy="125" r={isActive ? 3.5 : 2} fill={isActive ? '#a855f7' : '#475569'} className="transition-all duration-300" />
                </g>
              );
            })()}
=======
