...
def generate_pdf_report(report, user, output_path: str):
    ...
    pdf.cell(70, 6, user.name, ln=False)
    
    pdf.set_font("Helvetica", "B", 9.5)
    pdf.cell(25, 6, "Patient Email:", ln=False)
    pdf.set_font("Helvetica", "", 9.5)
    pdf.cell(70, 6, user.email, ln=True)
    ...
    pdf.set_xy(14, y + 3.5)
    pdf.cell(0, 6, result_text, ln=True)
    ...
    # 3. Medical Visualizations Side-by-Side
    pdf.set_font("Helvetica", "B", 10.5)
    pdf.set_text_color(45, 55, 72)
    pdf.cell(0, 6, "III. SCANS & GRAD-CAM VISUALIZATIONS", ln=True)
    pdf.ln(2)
